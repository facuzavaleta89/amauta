...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const rl = await rateLimit(request, { key: `certificados_anular:${user.id}`, limit: 10, windowMs: 60_000 })
    ...
    // Actualizar estado del certificado a 'revocado'
    const { data, error } = await supabase
      .from('certificados')
      .update({ estado: 'revocado' })
      .eq('id', id)
      .eq('firmado_por', user.id) // El médico es dueño de sus certificados firmados
      .select()
      .single()

    if (error || !data) {
      console.error('[POST /api/certificados/[id]/anular] Error:', error)
      return NextResponse.json({ error: 'Certificado no encontrado o sin permisos' }, { status: 404 })
    }

    return NextResponse.json({ data })
  } catch (err) {
    console.error('[POST /api/certificados/[id]/anular]', err)
